...
namespace TrashFormsdNotFinal
{
    public class DatabaseManager
    {
        ...
        {
            string connString = $"server={dbUrl};uid={username};pwd={password};database={database}";
            try
            {
        ...
        {
            string query = "INSERT INTO Users (username, user_password, user_role) VALUES (@username, @user_password, @user_role)";
        ...
        {
            string query = "SELECT * FROM Users WHERE username = @username AND user_password = @user_password";
        ...
        public void InsertWasteItem(int userId, string itemName, int quantity, string wasteType)
        {
            ...
            try
            {
                using (var cmd = new MySqlCommand(query, connection))
                {
                    ...
                    cmd.ExecuteNonQuery();
                    Console.WriteLine("Waste item added successfully!");
                }
            }
            catch (MySqlException e)
            {
                Console.WriteLine("Error inserting waste item: " + e.Message);
            }
        }
        ...
        public void DisplayRecyclingSummary(int userId)
        {
                ...
                using (var cmd = new MySqlCommand(query, connection))
                {
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            ...
                            Console.WriteLine($"{wasteType}: {totalQuantity} items recycled.");
                        }
                    }
                }
            }
            ...
            {
                Console.WriteLine("Error displaying recycling summary: " + e.Message);
            }
        }
